...
export const LoginPage: React.FC = () => {
  ...
  const processGoogleCredential = async (credential: string) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || err.message || 'Đăng nhập Google thất bại. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[85vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50/60">
      <div className="max-w-md w-full space-y-6">
        ...
        <div className="bg-white rounded-3xl p-7 sm:p-8 border border-slate-200/80 shadow-xl shadow-slate-900/5 space-y-5">
          ...
          <div id="google-official-btn" className="w-full flex justify-center min-h-[44px]"></div>

          {/* Fallback Google Sign-in Trigger Button (Only displayed if GIS has not loaded) */}
          {!gisReady && (
            <button
              type="button"
